/* ================= v5.0 Script =================
   Changes from v4.1:
   - New EV (Benefits Verification) tab — checklist-style generator, separate
     from the sentence-style Claim/PA generators. Payer Name/Phone, Member ID,
     Payer ID, Eligibility (Active for DOS + From/Till), Plan Type, In-Network,
     Copay, Deductible (Total/Met/Remaining), Coinsurance, OOP Max
     (Total/Met/Remaining), Referral Required, Rep Name, Call Reference #.
   - Every EV section has an include/exclude checkbox — uncheck to leave it
     out of the comment (same idea as unchecking a CPT chip in PA).
   - "+ Add Custom Line" lets you add any extra checklist line without a code change.
   - Copay/Deductible/OOP Max auto-add $, Coinsurance auto-adds %.
   ================= v4.1 changes =================
   Changes from v4.0:
...
window.addEventListener('load', () => {
  ...
  initEvForm();          // set up EV tab (checkbox toggles, listeners)
});

/* ================= Claim / PA / EV Mode Switch ================= */
const modeClaimBtn = document.getElementById('modeClaimBtn');
const modePaBtn = document.getElementById('modePaBtn');
const modeEvBtn = document.getElementById('modeEvBtn');
const claimSection = document.getElementById('claimSection');
const paSection = document.getElementById('paSection');
const evSection = document.getElementById('evSection');
const mainEl = document.querySelector('main');

const MODE_BUTTONS = { claim: modeClaimBtn, pa: modePaBtn, ev: modeEvBtn };
const MODE_SECTIONS = { claim: claimSection, pa: paSection, ev: evSection };

function switchMode(mode) {
  Object.keys(MODE_BUTTONS).forEach(key => {
    const isActive = key === mode;
    MODE_BUTTONS[key].classList.toggle('active', isActive);
    MODE_BUTTONS[key].setAttribute('aria-selected', isActive ? 'true' : 'false');
    MODE_SECTIONS[key].style.display = isActive ? '' : 'none';
  });
  mainEl.classList.toggle('pa-mode', mode === 'pa');
  mainEl.classList.toggle('ev-mode', mode === 'ev');
}

modeClaimBtn.addEventListener('click', () => switchMode('claim'));
modePaBtn.addEventListener('click', () => switchMode('pa'));
modeEvBtn.addEventListener('click', () => switchMode('ev'));
...
/* ================= EV (Benefits Verification) Form =================
   A checklist-style generator, not a sentence generator. Each section has an
   "include" checkbox — uncheck it (like removing a PA chip) and that line is
   left out of the comment entirely. "+ Add Custom Line" covers anything not
   in the default checklist (Out-of-Network deductible, secondary payer, etc.)
   without ever needing a code change.
*/

// Generic $ / % auto-formatters (same idea as the PA tab's formatPrAmount)
function formatMoney(rawVal) {
...
// Dim a checklist block when its "include" checkbox is unchecked, for visual feedback.
function wireEvBlockToggle(checkboxId) {
  const checkbox = document.getElementById(checkboxId);
  const block = checkbox.closest('.ev-block');
  const applyState = () => block.classList.toggle('disabled-block', !checkbox.checked);
  checkbox.addEventListener('change', applyState);
  applyState();
}

document.getElementById('evPlanType').addEventListener('change', function () {
  document.getElementById('evPlanOtherContainer').innerHTML = this.value === 'other'
    ? `<input id="evPlanOtherText" type="text" placeholder="Custom plan type" />` : '';
});

/* ---- Custom checklist lines ---- */
function addEvCustomLine() {
  const container = document.getElementById('evCustomLines');
  const row = document.createElement('div');
  row.className = 'ref-row';
  row.innerHTML = `
    <div class="row-line">
      <input class="evCustomLabel" type="text" placeholder="Label (e.g., Out-of-Network Deductible)" />
      <input class="evCustomValue" type="text" placeholder="Value" />
    </div>
    <button type="button" class="remove-row-btn" title="Remove">×</button>
  `;
  container.appendChild(row);
}
document.getElementById('addEvCustomLineBtn').addEventListener('click', addEvCustomLine);
document.getElementById('evCustomLines').addEventListener('click', (e) => {
  if (e.target.matches('.remove-row-btn')) e.target.closest('.ref-row').remove();
});

/* ---- Build the "Total | Met | Remaining" style line (Deductible / OOP Max) ---- */
function buildMoneyTripleLine(label, totalId, metId, remainingId) {
  const total = formatMoney(document.getElementById(totalId).value);
  const met = formatMoney(document.getElementById(metId).value);
  const remaining = formatMoney(document.getElementById(remainingId).value);
  if (!total && !met && !remaining) return null;
  const segs = [];
  segs.push(`${label}: ${total || '____'}`);
  if (met) segs.push(`Met: ${met}`);
  if (remaining) segs.push(`Remaining: ${remaining}`);
  return segs.join(' | ');
}

/* ---- Build the full EV comment ---- */
function buildEvComment() {
  const payerName = document.getElementById('evPayerName').value.trim();
  if (!payerName) { alert('Please enter Payer Name.'); return null; }
  const payerPhone = document.getElementById('evPayerPhone').value.trim();
  const memberId = document.getElementById('evMemberId').value.trim();
  const payerId = document.getElementById('evPayerId').value.trim();

  const lines = [];

  let payerLine = `Payer: ${payerName}`;
  if (payerPhone) payerLine += ` @${payerPhone}`;
  lines.push(payerLine);
  if (memberId) lines.push(`Member ID: ${memberId}`);
  if (payerId) lines.push(`Payer ID: ${payerId}`);

  if (document.getElementById('evIncludeEligibility').checked) {
    const active = document.getElementById('evActiveDos').value;
    const from = document.getElementById('evFrom').value.trim();
    const till = document.getElementById('evTill').value.trim();
    if (active) lines.push(`Eligibility: Active for DOS? ${active}`);
    if (from || till) lines.push(`from ${from || '---'} till ${till || '---'}`);
  }

  if (document.getElementById('evIncludePlan').checked) {
    let plan = document.getElementById('evPlanType').value;
    if (plan === 'other') plan = document.getElementById('evPlanOtherText')?.value.trim() || '';
    if (plan) lines.push(`Plan Type: ${plan}`);
  }

  if (document.getElementById('evIncludeNetwork').checked) {
    const inNet = document.getElementById('evInNetwork').value;
    if (inNet) lines.push(`In-Network: ${inNet}`);
  }

  if (document.getElementById('evIncludeCopay').checked) {
    const copay = formatMoney(document.getElementById('evCopay').value);
    if (copay) lines.push(`Copay: ${copay}`);
  }

  if (document.getElementById('evIncludeDeductible').checked) {
    const line = buildMoneyTripleLine('Deductible', 'evDedTotal', 'evDedMet', 'evDedRemaining');
    if (line) lines.push(line);
  }

  if (document.getElementById('evIncludeCoins').checked) {
    const coins = formatPercent(document.getElementById('evCoinsurance').value);
    if (coins) lines.push(`Coinsurance: ${coins}`);
  }

  if (document.getElementById('evIncludeOop').checked) {
    const line = buildMoneyTripleLine('OOP Max', 'evOopTotal', 'evOopMet', 'evOopRemaining');
    if (line) lines.push(line);
  }

  if (document.getElementById('evIncludeReferral').checked) {
    const referral = document.getElementById('evReferral').value;
    if (referral) lines.push(`Referral Required: ${referral}`);
  }

  const repName = document.getElementById('evRepName').value.trim();
  if (repName) lines.push(`Rep Name: ${repName}`);
  const callRef = document.getElementById('evCallRef').value.trim();
  if (callRef) lines.push(`Call Reference #: ${callRef}`);

  document.querySelectorAll('#evCustomLines .ref-row').forEach(row => {
    const label = row.querySelector('.evCustomLabel').value.trim();
    const value = row.querySelector('.evCustomValue').value.trim();
    if (!value) return;
...
document.getElementById('generateEvBtn').addEventListener('click', () => {
  const comment = buildEvComment();
  if (comment === null) return;
...
function initEvForm() {
  [
    'evIncludeEligibility', 'evIncludePlan', 'evIncludeNetwork', 'evIncludeCopay',
    'evIncludeDeductible', 'evIncludeCoins', 'evIncludeOop', 'evIncludeReferral'
  ].forEach(wireEvBlockToggle);
}
// ================= End of Script =================
